...
local pattern = TS.import(script, game:GetService("ReplicatedStorage"), "TS", "modules", "pattern").default
local path
do
	path = {}
	...
	function path:last(pathlike)
		local z = string.split(pathlike, "&")[1]
		local paths = string.split(z, "//")
		return paths[#paths - 1 + 1]
	end
	function path:getInstance(pathlike)
		local p = string.split(pathlike, "&")[1]
		local paths = string.split(p, "//")
		local inst = game
		local err = false
